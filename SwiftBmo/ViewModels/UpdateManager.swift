...
final class UpdateManager: ObservableObject {
        ...
        // For macOS, if it's a DMG, auto-install; otherwise, open the file
          ...
          if destination.pathExtension.lowercased() == "dmg" {
            do {
              ...
              let installedAppURL = URL(fileURLWithPath: "/Applications").appendingPathComponent(
  ...
  private func performRequest(url: URL, manual: Bool) async {
    ...
    do {
      ...
      if let gh = ghRelease {
        ...
        let assetURL =
          selectAssetURL(from: gh.assets, preferredExtensions: preferred) ?? gh.assets.first?
          .browser_download_url ?? gh.html_url ?? url

        let mapped = UpdateInfo(
          version: version,
          build: build,
          url: assetURL,
  ...
  private func mountDMG(at url: URL) throws -> URL {
    ...
    if task.terminationStatus != 0 {
      throw NSError(
        domain: "HDIUtilError", code: Int(task.terminationStatus),
    ...
    throw NSError(
      domain: "MountError", code: 1,
      userInfo: [NSLocalizedDescriptionKey: "Unable to determine mount point"])
  }
  ...
  private func unmountDMG(at url: URL) throws {
    ...
    if task.terminationStatus != 0 {
      throw NSError(
        domain: "HDIUtilError", code: Int(task.terminationStatus),
        userInfo: [NSLocalizedDescriptionKey: "Failed to unmount DMG"])
    }
  }
  ...
  private func findAppInVolume(at volumeURL: URL) throws -> URL {
    ...
    guard let enumerator = fileManager.enumerator(at: volumeURL, includingPropertiesForKeys: nil)
    else {
      throw NSError(
        domain: "UpdateManager", code: 1,
        userInfo: [NSLocalizedDescriptionKey: "Failed to enumerate volume"])
    }
    ...
    throw NSError(
      domain: "UpdateManager", code: 0,
      userInfo: [NSLocalizedDescriptionKey: "No application bundle found in DMG"])
  }
  ...
  private func installApp(from url: URL) throws {
    ...
    let applicationsDirectory = fileManager.urls(for: .applicationDirectory, in: .userDomainMask)
      .first!
